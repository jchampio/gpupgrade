...
type MultinodeBatsJob struct {
	Source, Target string
	CentosVersion  string
}

func (j *MultinodeBatsJob) Name() string {
	return fmt.Sprintf("%s-centos-%s", j.BaseName(), j.CentosVersion)
}
...
// This is used as a tag in Concourse's serial group to limit similar jobs
// between operating systems from running at once to avoid overloading Concourse.
func (j *MultinodeBatsJob) BaseName() string {
	return fmt.Sprintf("%s-to-%s-%s", j.Source, j.Target, "multinodeBats")
}
...
type Data struct {
	...
	Versions          []*Version
	CheckJobs         []*CheckJob
	MultinodeBatsJobs []*MultinodeBatsJob
}
...
func init() {
	...
	var multinodeBatsJobs []*MultinodeBatsJob
	for _, sourceVersion := range sourceVersions {
		for _, targetVersion := range targetVersions {
			...
			for _, centosVersion := range centosVersions {
				...
				multinodeBatsJobs = append(multinodeBatsJobs, &MultinodeBatsJob{
					Source:        sourceVersion,
					Target:        targetVersion,
					CentosVersion: centosVersion,
				})
	...
	data = Data{
		...
		Versions:          versions,
		CheckJobs:         checkJobs,
		MultinodeBatsJobs: multinodeBatsJobs,
	}
}
